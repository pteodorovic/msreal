...
static void write(char* str)
{
    ...
    br=strToInt(str, len, 10);
	 ...
	 if(offset>=BUFF_SIZE)
	 {
		printk("Buffer is full, going to sleep\n");
		wait_event_interruptible(wq, offset < BUFF_SIZE);
	 }
	 *pLast=br;
...
static void read(void)
{
	...
	if(offset==0)
	{
		printk("Buffer empty, going to sleep\n");
		wait_event_interruptible(rq, offset != 0);
	}
	printk("%d\n",*pFirst);
